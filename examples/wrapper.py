...
def main_handler (http_receive, request):
	cerver.http_send_response (http_receive, cerver.HTTP_STATUS_OK, { "msg": "Wrapper works!" })
...
def echo_handler (http_receive, request):
	...
	cerver.http_send_response (http_receive, cerver.HTTP_STATUS_OK, { "echo_says": f"Wrapper received: {value}" })

# POST /data
@ctypes.CFUNCTYPE (None, ctypes.c_void_p, ctypes.c_void_p)
def data_handler (http_receive, request):
	body = cerver.http_request_get_body_json (request)
	cerver.http_send_response (http_receive, cerver.HTTP_STATUS_OK, { "echo": body })
